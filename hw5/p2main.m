...
p2_2=network('p2.config');
p2_3=network('p2.config');
...
testx_2 = (testinput_2+0.2*(testinput_2.^2)-0.8)/2;

testy_2 = (testinput_2)/2;
...
%trainmatrix=p2.train(trainx,trainy,0.9,0.01,1);%BPlearn
%input, output, momentum,learning rate,learning steps(per epoch)

for i=1:60
    trainmatrix=p2.train(trainx,trainy,0.9,0.01,50);%BPlearn
    er_train=p2.test(trainx,trainy)*p2.test(trainx,trainy); 
    %BPrecall(train), what we have is RMSD, so we need to calculate the
    %square
    eror_train=[eror_train,er_train];
  
    er_test1=p2.test(testx_1,testy_1)*p2.test(testx_1,testy_1); %BPrecall(test)for test1
    eror_test1=[eror_test1,er_test1];
    
    er_test2=p2.test(testx_2,testy_2)*p2.test(testx_2,testy_2); %BPrecall(test)for test2
    eror_test2=[eror_test2,er_test2];
    
    er_test3=p2.test(testx_3,testy_3)*p2.test(testx_3,testy_3); %BPrecall(test)for test3
...
%2
for i=1:60
    trainmatrix_2=p2_2.train(trainx,trainy,0.9,0.015,50);%BPlearn
    er_train_2=p2_2.test(trainx,trainy)*p2_2.test(trainx,trainy); 
    %BPrecall(train), what we have is RMSD, so we need to calculate the
    %square
    eror_train2=[eror_train2,er_train_2];
  
    er_test1=p2_2.test(testx_1,testy_1); %BPrecall(test)for test1
    eror_test1=[eror_test1,er_test1];
    
    er_test2=p2_2.test(testx_2,testy_2); %BPrecall(test)for test2
    eror_test2=[eror_test2,er_test2];
    
    er_test3=p2_2.test(testx_3,testy_3); %BPrecall(test)for test3
    eror_test3=[eror_test3,er_test3];
...
%3
for i=1:60
    trainmatrix_3=p2_3.train(trainx,trainy,0.9,0.05,50);%BPlearn
    er_train_3=p2_3.test(trainx,trainy)*p2_3.test(trainx,trainy); 
    %BPrecall(train), what we have is RMSD, so we need to calculate the
    %square
    eror_train3=[eror_train3,er_train_3];
  
    er_test1=p2_3.test(testx_1,testy_1); %BPrecall(test)for test1
    eror_test1=[eror_test1,er_test1];
    
    er_test2=p2_3.test(testx_2,testy_2); %BPrecall(test)for test2
    eror_test2=[eror_test2,er_test2];
    
    er_test3=p2_3.test(testx_3,testy_3); %BPrecall(test)for test3
    eror_test3=[eror_test3,er_test3];
...
recalltrain_rs = p2.recall(trainx)*2;
trainy_rs=trainy*2;

testx_1_rs =testx_1*2+0.8;
recall_1_rs = p2.recall(testx_1)*2;

testx_2_rs =testx_2*1.016+0.19;
recall_2_rs = p2.recall(testx_2)*1.023-0.023;
...
hold on
%plot(testx_2,p2.recall(testx_2),'+g','MarkerEdgeColor','g','MarkerFaceColor','g','MarkerSize',8);
plot(testx_3,p2.recall(testx_3),'xk','MarkerEdgeColor','k','MarkerFaceColor','k','MarkerSize',8);

plot(trainx_v,trainy_v,'-b','MarkerEdgeColor','b','MarkerFaceColor','b','MarkerSize',4);
%plot(testx_3,p2.recall(testx_3),'*g','MarkerEdgeColor','g','MarkerFaceColor','g','MarkerSize',4);
...
plot(testx,p2.recall(testx),'xc','MarkerEdgeColor','c','MarkerFaceColor','c','MarkerSize',4);
...
plot(testx,p3.recall(testx),'+r','MarkerEdgeColor','r','MarkerFaceColor','r','MarkerSize',4);
hold on
plot(testx,p2.recall(testx),'xk','MarkerEdgeColor','k','MarkerFaceColor','k','MarkerSize',4);
plot(testx,p6.recall(testx),'*m','MarkerEdgeColor','m','MarkerFaceColor','m','MarkerSize',4);
